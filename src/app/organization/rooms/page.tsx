...
export default function RoomsPage() {
  ...
  const handleDelete = (id: string, roomName: string) => {
    ...
    if (linkedAssets.length > 0) {
      alert(
        `Deletion Not Permitted: Room "${roomName}" (${id}) cannot be deleted because it has ${linkedAssets.length} asset(s) installed (${linkedAssets.map(a => a.name).join(', ')}). Please reassign or delete those assets first.`
      )
      return
    }

    if (confirm(`Are you sure you want to delete room "${roomName}" (${id})?`)) {
      deleteRoom(id)
    }
  }
              ...
              <div key={r.id} className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-2xs space-y-3 hover:border-slate-300 transition group">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${r.status === 'Occupied' ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200'}`}>
                      {r.status}
                    </span>
                ...
                <div>
                  ...
                  <Link href={`/organization/rooms/${r.id}`}>
