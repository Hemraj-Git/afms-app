...
- [ ] **5.14 Room access logs / check-in-out** — check in and out as guest and as Admin; log entries show the **room name** (and follow a renamed room); room shows Occupied then Available. **Two different guests can check in one after another with no error** (each entry gets its own AL-A#### number, assigned by the database).
...
- [ ] **6.7 Open-work badges** — On `/mobile` as a Technician with a mix of Scheduled / In Progress, Completed and Cancelled corrective/preventive orders: the **Tasks** tab badge and the first summary card ("Open Work Orders") count **only Scheduled + In Progress** orders. Completing an order lowers the number at once; a Cancelled order is not counted and is not in the *All* list. Same for a Housekeeping user: the **Cleaning** badge and "Open Orders" card. The Completed filter still lists finished orders.
- [ ] **6.8 "PWA" wording removed** — The mobile header reads **Field Operations** (no "PWA"), the loading screen says "Loading Field Operations…", and the desktop header link is **Field Mode**.
- [ ] **6.9 Vendor handover — technician (mobile)** — Open a *Corrective* order → **Handover to Vendor**. The vendor list starts on **"-- Select the vendor --"** unless the asset has its own maintenance vendor (then that one is preselected). There is **no start-photo box** in this mode. **Save as In Progress** without a vendor → refused. **Complete** without a *Vendor Ticket / Job No* → refused; with vendor + ticket number → completes (asset Operational, ticket Resolved) with no start photo. The **Vendor Cost** box accepts an amount (no negatives) and keeps it after reload. Switching back to *In-House* still requires the start photo and completion photo to complete.
- [ ] **6.10 Vendor handover — Admin visibility** 🔴 — Two windows: Admin on the desktop, technician on `/mobile`. The technician hands a corrective order to a vendor (Save as In Progress). The **Admin's bell gets a "Handed to vendor: WO-CR-…" alert with a chime** (the vendor name is in the text); clicking it opens *Corrective Maintenance* and marks it read. The order shows a **"With vendor"** tag on the desktop *Corrective* and *Work Orders* lists and on the technician's mobile card. Opening the order's details shows a **"Handed over to vendor"** block: vendor, ticket no., vendor technician (tappable phone), visit date, cost in ₹, remarks and the job-sheet photo. Editing the vendor details afterwards, or completing the job, sends **no second alert**; the tag disappears once it is Completed. An Admin who makes the handover does not alert themselves. The desktop bell now also lists other alerts addressed to you (work / inspection assigned) and "Clear all" marks them read.
- [ ] **6.11 Asset status goes live** 🔴 — Two windows: Admin on *Assets* (or an asset's detail page), technician on `/mobile`. Admin raises a Corrective work order for an asset and assigns the technician → the asset flips to **Under Maintenance** in the Admin window without a refresh. The technician completes the order → the asset flips back to **Operational**, the ticket shows **Resolved**, and a new "Corrective Maintenance Completed" entry appears in the asset's *Activity & History* tab, all **without refreshing** the Admin window. (Resolving a service request by hand does not change the asset — only completing its work order does.)
- [ ] **6.12 Guest re-login sees their own open check-in** — On `/mobile`, check in as a guest (leave a purpose), then sign out and sign back in with the **same email**. The "Active in: …" banner and Check Out button appear even though this is a fresh login — the app now recognises it as the same guest's still-open check-in, not just the exact account that made it. Check out → the room goes Available and the log shows a check-out time. A **different** email never sees someone else's check-in.
- [ ] **6.13 A room can hold more than one guest** — Two different guest emails check into the **same** room without either check-out. The room stays **Occupied** and shows the **most recently arrived** person's name. The first person checks out → the room **stays Occupied**, now showing the other person's name (not Available). Only once the last person checks out does the room go **Available**. (Previously any single check-out unconditionally freed the room, even with someone else still in it.)
